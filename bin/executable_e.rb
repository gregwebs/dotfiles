...
[ 'XZ archive', '.tar.xz',    fn{|f| [:system, "xz --decompress #{f} | tar -xv"]} ],
[ 'XZ compression', '.xz',    fn{|f| "xz --decompress #{f}"} ],
...
def help; <<-EOS
  usage: e archive [ archive2 archive3 ... ]
	  ...
	  --dry-run     show the command that would be run
          -             accept piped input
EOS
end
...
def check_files(files)
  files.map do |file|
    ...
    case file
      ...
      when /^--type/
        exit_msg(re_to_text( $rules.map {|r| r[0]} ), 0)
      when /^--ext/
	exit_msg(re_to_text( $rules.map {|r| r[1][3..-3] if r[1]} ), 0)
      when /^--help/
	exit_msg(help(), 0)
      when /^--dry-run/
	$dry_run = true
	next nil
      ...
      when /^--?\w*$/
        ...
        exit_msg("archive does not exist: #{file}", 1)
      end
  end.compact
end
...
def match_and_extract( filename, matcher, rule_set )
  rule_set.find { |regex, command_proc|
    if( matcher.match( regex ) )
      ...
      if $dry_run
	if method(runner).arity == 2
          puts([runner, command, filename].inspect)
	else
          puts([runner, command].inspect)
	end
        next true
      end
